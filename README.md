...
- 修复课程页卡死: "学习时长/学前必读"弹窗(`.studytime-div`)已读时是常驻 DOM 的隐藏节点, 等它可见会一直等下去; 可选元素的等待改为有上限, 超时按"未出现"跳过(社区 PR #155).
- 课时切换与目录识别前的等待补上超时, 异常页面下不再长时间卡住.
- 支持 Python 3.13, 并按解释器选择匹配的 numpy / opencv 依赖版本.
